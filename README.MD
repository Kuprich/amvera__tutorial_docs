...
После загруки кода в удаленный репозиторий начнется сборка проекта, которая завершится с ошибкой. Необходимо выполнить конфигурацию проекта. Перейдите к этипу 3, нажав кнопку *"Далее"*:
...
```$ git pull amvera master``` для получения файла *amvera.yml*:
